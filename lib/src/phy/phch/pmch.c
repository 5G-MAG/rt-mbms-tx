                                     ...
                                     uint32_t* sym_re, uint32_t max_syms)
{
    ...
    /* SL4 type-1: NRE_SCS=486 is not a multiple of 12, so the local RS start
     * within PRB n shifts by (n*6)%12.  The total data RE in the RS symbol
     * differs from prb_count*(nre-41) by up to floor(prb_count/2) REs.
     * Use a closed-form over the two alternating per-PRB RS counts. */
    ...
    if (scs == SRSRAN_SCS_370HZ_SL4) {
        ...
        uint32_t rs1  = (nre - s1 + 11u) / 12u;  /* per-PRB RS: even-indexed PRBs */
        uint32_t rs2  = (nre - s2 + 11u) / 12u;  /* per-PRB RS: odd-indexed PRBs */
        uint32_t n1   = (prb_count + 1u) / 2u;   /* ceil(prb_count/2) */
        uint32_t n2   = prb_count / 2u;           /* floor(prb_count/2) */
        sl4_rs_sym_re = prb_count * nre - (n1 * rs1 + n2 * rs2);
    }
        ...
        for (uint32_t l = ls; l < SRSRAN_MBSFN_NOF_SYMBOLS(scs); l++) {
            if (L < max_syms) {
                if (SRSRAN_SYMBOL_HAS_REF_MBSFN_SCS(l, s, scs)) {
                    sym_re[L] = (scs == SRSRAN_SCS_370HZ_SL4) ? sl4_rs_sym_re
                                                                : prb_count * (nre - nof_refs);
                } else {
                    sym_re[L] = prb_count * nre;
                }
            }
            L++;
        }
...
/* TS 36.211 §6.5.1: apply right-cyclic-shift of Xi bits to the packed byte array.
 * b'[n] = b[(n - Xi) mod Mbit_bits].
 * bit_buf is a packed byte array of Mbit_bits bits. */
/* TS 36.211 §6.5.1: left cyclic shift — d'(k) = d((k + Xi) mod Mbit).
 * TX applies this after rate matching, before scrambling.
 * RX undoes it (shift by Mbit-Xi) after descrambling, before decoding. */
static void pmch_cyclic_shift_bits(uint8_t* bit_buf, uint32_t Mbit_bits, uint32_t Xi)
{
    ...
    /* Left shift: first Xi bits move to the back. out[0..Mbit-Xi-1] = in[Xi..Mbit-1], out[Mbit-Xi..Mbit-1] = in[0..Xi-1]. */
 ...
 * LLR'[k] = LLR[(k + anti_Xi) % Mbit]  where anti_Xi = Mbit - Xi. */
...
static uint32_t pmch_cyclic_shift_Xi(uint32_t Mbit_bits, uint32_t C, uint32_t i, uint32_t alpha)
{
    ...
    /* Er[r] = E_floor + 1 for r >= C - rem, else E_floor */
    uint32_t Xi = 0;
 ...
 * "row r_i is moved to row i" means: permuted row i = original row perm[i]. */
static void pmch_interleave_row_perm(uint32_t R, uint32_t K, uint32_t* perm)
{
    ...
    if (inner < 1) inner = 1; /* guard against K > sqrt(RK)+1 edge case */
    ...
    for (uint32_t i = 1; i < R; i++) {
        if (perm[i-1] + d <= R - 1) { /* r_{i-1} <= R - d - 1 */
            perm[i] = perm[i-1] + d;
        } else {
            perm[i] = (uint32_t)(((uint64_t)i * d) / R); /* floor(i*d / R) */
        }
    }
}

/* Column cyclic shift offset for post-permutation row r:
 * sr = (-1)^r * floor((r+1)/2) mod K  (interpreted as non-negative) */
static uint32_t pmch_col_shift(uint32_t r, uint32_t K)
{
    if (K == 0) return 0;
    int s = (int)((r + 1) / 2);      /* floor((r+1)/2) */
    if (r % 2 == 1) s = -s;          /* (-1)^r */
    int sr = s % (int)K;
    if (sr < 0) sr += (int)K;
    return (uint32_t)sr;
                                  ...
                                  const uint32_t* sym_re, bool inverse)
{
    ...
    for (uint32_t l = 0; l < L; l++) {
        ...
        /* Compute row permutation and column shifts */
        ...
        /* NULL mask: grid_null[r*K+c] = true if that cell is a padding NULL */
        bool* grid_null = calloc(grid_sz, sizeof(bool));

        if (!inverse) {
            /* TX: write col-wise → row-perm → col-shift → read row-wise */

            /* Step 1: write column-wise into grid.
             * Input index i → (col = i/R, row = i%R) → grid[row*K + col]. */
            ...
            /* Step 2: row permutation (permute into grid2). */
            ...
            /* Step 3: cyclic column shift per row. */
            ...
            /* Step 4: read row-wise, discarding NULLs. */
            uint32_t out_idx = 0;
            ...
            /* First, build the NULL mask in the final (post-shift, post-perm) grid.
             * NULL positions originate from the padding region (original col-wise index ≥ Ml).
             * Original (pre-perm) col-wise index i: row_orig = i%R, col_orig = i/R.
             * After perm: ends up at row j where perm[j] = row_orig → pre-perm_inv.
             * After col shift of row j by sr[j]: ends up at col (col_orig + sr[j]) % K.
             * Final position: (row = j, col = (col_orig + sr[j]) % K). */
            ...
            /* Step 1: write received symbols row-wise into non-NULL positions. */
            uint32_t in_idx = 0;
            ...
            /* Step 2: undo column shift per row. */
            cf_t* grid2 = malloc(grid_sz * sizeof(cf_t));
            for (uint32_t r = 0; r < R; r++) {
                uint32_t s = sr[r];
                if (s == 0) {
            ...
            /* Step 3: undo row permutation. */
            for (uint32_t i = 0; i < R; i++) {
                memcpy(&grid[perm[i] * K], &grid2[i * K], K * sizeof(cf_t));
            }
            free(grid2);

            /* Step 4: read column-wise (Ml symbols). */
...
int srsran_pmch_init(srsran_pmch_t* q, uint32_t max_prb, uint32_t nof_rx_antennas)
{
  ...
  if (q != NULL && nof_rx_antennas <= SRSRAN_MAX_PORTS) {
    ...
    q->cell.nof_ports  = 1;
    /* Size for the widest MBSFN SCS per 1 ms subframe.
     * 0.37 kHz (SL2/SL4): 1 symbol/subframe × 486 sc/PRB = 486 RE/PRB (widest). */
    q->max_re          = max_prb * MAX_PMCH_RE(SRSRAN_SCS_370HZ);
...
int srsran_pmch_set_cell(srsran_pmch_t* q, srsran_cell_t cell)
{
  ...
  if (q != NULL && srsran_cell_isvalid(&cell)) {
    q->cell   = cell;
    uint32_t prb = q->cell.mbsfn_prb > 0 ? q->cell.mbsfn_prb : q->cell.nof_prb;
    q->max_re = prb * MAX_PMCH_RE(SRSRAN_SCS_370HZ);
                       ...
                       srsran_pdsch_res_t*    out)
{
  ...
  if (q != NULL && sf_symbols != NULL && out != NULL && cfg != NULL) {
    ...
    /* TS 36.211 §6.5.1: undo bit-level left cyclic shift (anti-shift = shift by Mbit-Xi). */
    ...
    /* TS 36.213 §11.1 (see the matching comment in srsran_pmch_encode above
                       ...
                       uint8_t**           shared_ti_tx_buf)
{
  ...
  if (q != NULL && cfg != NULL) {
    ...
    uint32_t Mbit_sf = cfg->pdsch_cfg.grant.tb[0].nof_bits;
    ...
    if (N > 1) {
       ...
       * across its own N calls: MAC only supplies a non-NULL data pointer at
       * n==0 for slot m (see mac.cc), so n=1..N-1 need the SAME payload -
       ...
       * comment in pmch.h for why a single-worker q->ti_tx_buf[] is unsafe
       * when multiple worker threads each hold their own srsran_pmch_t. */
      uint8_t* slot_buf;
      if (shared_ti_tx_buf != NULL) {
        slot_buf = shared_ti_tx_buf[slot_m];
        if (!slot_buf) {
          ERROR("shared_ti_tx_buf[%d] not allocated by caller", slot_m);
          return SRSRAN_ERROR;
        }
      } else {
        ...
        slot_buf = q->ti_tx_buf[slot_m];
      }
      if (getenv("PMCH_TI_DIAG")) {
        fprintf(stderr,
                "TI_DIAG_TX q=%p subframe_idx=%u slot_m=%u slot_n=%u data=%p tbs=%d shared=%d\n",
                (void*)q, cfg->subframe_idx, slot_m, slot_n, (void*)data, cfg->pdsch_cfg.grant.tb[0].tbs,
                shared_ti_tx_buf != NULL);
      }
      if (slot_n == 0 && data != NULL) {
        memcpy(slot_buf, data, ((uint32_t)cfg->pdsch_cfg.grant.tb[0].tbs + 7) / 8);
      }
      if (srsran_dlsch_encode_mch(&q->dl_sch, &cfg->pdsch_cfg, slot_buf, q->e, slot_n, e_min, n_cb_cap)) {
    ...
    /* TS 36.211 §6.5.1: bit-level left cyclic shift after rate matching, before scrambling. */
    if (cfg->cyclic_shift && cfg->cyclic_shift_alpha > 0) {
      uint32_t Xi;
      if (cfg->cyclic_shift_alpha == 3) {
        uint32_t Qm_bits = srsran_mod_bits_x_symbol(cfg->pdsch_cfg.grant.tb[0].mod);
     ...
     * symbol array (pre-RE-mapping, i.e. exactly what pmch_put() took as input).
     * This is the correct apples-to-apples reference for RX's post-equalization
    ...
    if (pmch_re_dump_enabled(sf->tti)) {
      ...
      if (fds) {
        fwrite(q->symbols[0], sizeof(cf_t), cfg->pdsch_cfg.grant.nof_re, fds);
        fclose(fds);
      }
      fprintf(stderr, "[PMCH_RE_DUMP] TX datasym tti=%u nof_re=%u\n", sf->tti, cfg->pdsch_cfg.grant.nof_re);
    }

    /* PMCH_RE_DUMP: scratch instrumentation - dumps the full per-subframe RE grid
     * (data + RS together, whole carrier bandwidth) right after pmch_put() writes into
     * it - the last point before this grid gets IFFT'd and transmitted. Matches the
     * RX-side post-FFT dump added in ue_dl.c's srsran_ue_dl_decode_fft_estimate(), for
