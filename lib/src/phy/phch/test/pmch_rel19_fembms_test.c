...
static int run_single_sf_case(const rel19_case_t* tc, uint32_t nof_prb, uint32_t mcs_idx, uint16_t mbsfn_area_id)
{
  ...
  if (srsran_pmch_encode(&pmch, &dl_sf, &pmch_cfg, data_tx, tx_grid, NULL)) {
...
static int run_time_interleave_case(const rel19_case_t* tc, uint32_t nof_prb, uint32_t mcs_idx, uint16_t mbsfn_area_id)
{
  ...
  for (uint32_t s = 0; s < (uint32_t)N * M; s++) {
    ...
    if (srsran_pmch_encode(&pmch, &dl_sf, &pmch_cfg, enc_data, tx_grid, NULL)) {
 ...
 * this test file's own verification of pmch.c:901's reset trigger (cfg->subframe_idx
...
int main(int argc, char** argv)
{
  ...
  const rel19_case_t cases[] = {
       ...
       * to produce a genuinely BINDING cap (n_cb_cap=3912, below the ~6624-bit
       * uncapped K_w for this case's TBS) that still round-trips correctly - not
       * just "the field is accepted", but "encode really shrinks the circular
       * buffer and decode still recovers the payload byte-exact". A much smaller
       * cap (same category/beta at M=4) was also hand-verified to correctly FAIL
       * (discards code positions below the codeword's own raw length - an
       * expected information-theoretic consequence of an overly aggressive cap,
       * not a bug) - deliberately not encoded as a test case here, since a
       * "correctly fails" case doesn't fit this file's round-trip-must-succeed
       * pattern; see the N_cb capping implementation commit for that data point. */
