...
void parse_args(all_args_t* args, int argc, char* argv[])
{
  ...
  common.add_options()
    ...
    ("embms.nof_pmch", bpo::value<uint16_t>()->default_value(1)->notifier([args](uint16_t v) { args->stack.embms.nof_pmch = static_cast<uint8_t>(v); }), "Number of PMCHs (1-2; PMCH0 always carries MCCH). Only settable at startup: unlike control_server.cc's live embms.nof_pmch SET (which can add a PMCH1 signalled with only a fabricated placeholder session), this is the only way to get a real session's M1-U data actually delivered to PMCH1 -- gtpu.cc's m1u_handler learns embms.pmch1.session_teids only once, here, at startup.")
    ("embms.pmch1.mcs", bpo::value<uint16_t>(&args->stack.embms.pmch1.mcs)->default_value(9), "PMCH1's own MCS. Only used if embms.nof_pmch=2.")
    ("embms.pmch1.session_teids", bpo::value<string>(&args->stack.embms.pmch1.session_teids)->default_value(""), "PMCH1's own session_teids -- same format as embms.session_teids, but scoping which real M3AP sessions' data is delivered to PMCH1 specifically rather than PMCH0. Only used if embms.nof_pmch=2.")
    ("embms.pmch1.time_interleaving_n", bpo::value<uint16_t>()->default_value(0)->notifier([args](uint16_t v) { args->stack.embms.pmch1.time_interleaving_n = static_cast<uint8_t>(v); }), "PMCH1's own time_interleaving_n (0=disabled, 2/4/8/16) -- see embms.time_interleaving_n; PMCH1 never carries MCCH, so unlike PMCH0 this is never rejected. Only used if embms.nof_pmch=2.")
    ("embms.pmch1.time_interleaving_m", bpo::value<uint16_t>()->default_value(4)->notifier([args](uint16_t v) { args->stack.embms.pmch1.time_interleaving_m = static_cast<uint8_t>(v); }), "PMCH1's own time_interleaving_m (4/8/16/32) -- see embms.time_interleaving_m. Only used if embms.nof_pmch=2.")
    ("embms.pmch1.nof_mbms_sessions", bpo::value<uint16_t>()->default_value(1)->notifier([args](uint16_t v) { args->stack.embms.pmch1.nof_mbms_sessions = static_cast<uint8_t>(v); }), "PMCH1's own nof_mbms_sessions (1-8), used for its fabricated-fallback session count when embms.pmch1.session_teids doesn't (yet) match a real session. Only used if embms.nof_pmch=2.")
